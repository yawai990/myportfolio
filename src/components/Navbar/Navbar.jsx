import React from "react";
import { image } from "../constants";
import { Header } from '../../GlobalStyle';
import { Nav, NavHead, Group, Link, Icon, Thumbnail, Btn, Copy } from "../Style/Navbar";
import { Data } from './Data';
import { FaAngleDoubleLeft, FaAngleDoubleRight } from 'react-icons/fa';

const Navbar = ({ width, PlayNav, navActive }) => {
    return <Nav style={{ overflowY: `${navActive ? 'scroll' : 'hidden'}` }} width={width} py='2'>

        <NavHead active='true' style={{ marginBottom: '2rem' }}>

            <Btn onClick={PlayNav}>
                {navActive ? <FaAngleDoubleLeft /> : <FaAngleDoubleRight />}
            </Btn>
        </NavHead>

        <NavHead className="logo">
            <Thumbnail style={{
                width: navActive ? '120px' : '35px',
                height: navActive ? '120px' : '35px',
            }}>
                <img src='data:image/png;base64,' alt="" />
            </Thumbnail>

            <Header my='1' style={{ display: `${navActive ? 'block' : 'none'}` }}>Jonathan</Header>

            <Header fs='16' fw='500' style={{ display: `${navActive ? 'block' : 'none'}` }}>Junior Web Developer</Header>
        </NavHead>

        <Group>
            {Data.map(link => {
                return <Link href={`#${link.link}`} key={link.id} my='0.3'>
                    <Icon>{link.icon}</Icon>
                    <span style={{ display: navActive ? 'inline' : 'none' }}>{link.link}</span>
                </Link>
            })}
        </Group>

        <Copy style={{ display: `${navActive ? 'block' : 'none'}` }}>
            All rights reserverd.&copy; Copyright by Yawai.2022</Copy>
    </Nav>
}
export default Navbar;