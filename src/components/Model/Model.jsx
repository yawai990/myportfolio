...
import { Group, Link, Icon } from "../Style/Navbar";
import { Data } from '../Navbar/Data';
import { ModelContainer, Thumbnail, NavHead, Button, Copy } from '../Style/Model';
...
const Model = () => {
    ...
    return <ModelContainer style={{ transform: `translateX(-${model ? 100 : 0}%)` }}>
        ...
        <NavHead>
            ...
            <Header my='1'>Jonathan</Header>

            <Header fs='16' fw='500'>Junior Web Developer</Header>
        </NavHead>

        <Group>
            {Data.map(link => {
                return <motion.li key={link.id}
                    style={{ listStyle: 'none' }}
                    whileInView={{ x: [-100, 0], opacity: [0, 1] }}
                    transition={{ delay: link.id * 0.1 }}>
                    <Link href={`#${link.link}`} my='0.3' onClick={PlayModel}>
                        <Icon>{link.icon}</Icon>
                        <span>{link.link}</span>
                    </Link>
                </motion.li>
